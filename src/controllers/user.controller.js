...
const logoutUser = asyncHandler(async (req, res) => {
    ...
    await User.findOneAndUpdate(
        ...
        {
            $unset: {
                refreshToken: 1, // $set: { refreshToken: undefined } is not working here
            },
        },
        { new: true },
    );
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    if (!coverImage.url) {
        throw new ApiError(
            500,
            "error while cover image file updating on cloudinary",
        );
    }
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            $lookup: {
                ...
                localField: "_id", // current user id :- ObjectId('5f9c5b5b4c5b5b4c5b5b4c5b')
                foreignField: "channel", // channel id :- ObjectId('5f9c5b5b4c5b5b4c5b5b4c5b')
                as: "subscribers",
            },
        },
        {
        ...
        },
        {
            $addFields: {
                subscribeCount: {
                    $size: "$subscribers", // number of subscribers
                },
                ...
                pipeline: [
                    {
                        $lookup: {
                            ...
                            as: "owner",
                            pipeline: [
                                // TODO check later if project can be done in outside
                                {
                                    $project: {
                                        username: 1,
                                        fullName: 1,
